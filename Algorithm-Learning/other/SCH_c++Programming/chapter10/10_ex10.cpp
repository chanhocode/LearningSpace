/*
나라의 수도 맞추게 게임에 vector을 활용해보자. 나라 이름(nation)과 수도(capital)문자열로 구성된
Nation 클래스를 만들고, vector<Nation> v; 로 생성한 벡터를 이용하여 나라 이름과 수도 이름을 삭제할 수도 있고
랜덤하게 퀴즈를  볼수도있다. 프로그램 내에서 벡터에 Nation객체를 여러개 미리 삽입하여 퀴즈를 보도록하라.
*/
...
//                if (b)
//                    continue;
//                v.push_back(Nation(n, c)); // 정상 입력 값이면 vector v에 입력 
//            }
//            break;
//
//        case 2:
